...
seatTypeDAOImpl.save(st);
	...
	bookingDetailImpl.save(bd);
	Booking booking2=bookingDAOImpl.deleteBooking(107, 1);
	...
	assertThat(showlist2.size()).isGreaterThan(0);
	HallCapacity bd2=hallCapacityDAOImpl.find(5);
	assertThat(100).isEqualTo(bd2.getSeatCount());
	...
	assertThat(showlist2.size()).isGreaterThan(0);
	BookingDetail bd2=bookingDetailImpl.find(80);
	assertThat(20).isEqualTo(bd2.getNoOfSeats());
	}
	@Test
	void contextLoads12() {
   ...
   assertThat(true).isEqualTo(movie2 instanceof Movies); 
	...
	void contextLoads13() {
		...
		Shows test=showsDAOImpl.find(8);
		assertThat(10).isEqualTo(test.getSlotNo());
